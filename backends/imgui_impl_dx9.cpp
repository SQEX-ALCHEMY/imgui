...
//  2023-XX-XX: DirectX9: Add support for ImGuiBackendFlags_RendererHasTexReload.
...
static LPDIRECT3DTEXTURE9 ImGui_ImplDX9_UpdateTexture(LPDIRECT3DTEXTURE9 gpu_tex, const ImTextureData* in_tex_data)
{
    ...
    if (in_tex_data->TexFormat == ImTextureFormat_RGBA32)
    {
    ...
    if (gpu_tex == nullptr || surface_desc.Width != (UINT)width || surface_desc.Height != (UINT)height)
    {
        // Create texture
        gpu_tex = nullptr;
        if (bd->pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &gpu_tex, nullptr) < 0)
            return false;
